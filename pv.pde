...
boolean rightTurn(pt A, pt B, pt C) {
  vec N = R(V(A, B), PI/2.0);
  return dot(N, V(B, C)) >= 0;
}

boolean ptInTriangle(pt P, pt A, pt B, pt C)
{
  return rightTurn(P, A, B) == rightTurn(P, B, C) && rightTurn(P, B, C) == rightTurn(P, C, A);
}
